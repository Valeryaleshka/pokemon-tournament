...
const textColors: Record<PokemonType, string> = {
...
export class PokemonTypeDirective {
  ...
  constructor() {
    effect(() => {
        this.renderer.setStyle(this.elementRef.nativeElement, 'background-color', typeColors[this.appPokemonType()] ?? '#a4acaf');
        this.renderer.setStyle(this.elementRef.nativeElement, 'color', textColors[this.appPokemonType()] ?? '#000000');
